...
##########################################
def heuristic(y1, x1, x2, y2, path):
...
def push_switches(L_open, lines, columns, L_open_all):
    ...
    if push==1:
        aux=L_open[2]
        L_open=[L_open[0],L_open[1],L_open[2]+'P',L_open[3]+1,copy.deepcopy(L_open[4])]
...
def expand_nodes(L_open, L_open_foo, L_closed, x_goal, y_goal):
        ...
        if(L_open_foo[4][L_open_foo[0]+1][L_open_foo[1]] != 0 and L_open_foo[4][L_open_foo[0]+1][L_open_foo[1]] < 200):
            heu=heuristic(L_open_foo[0]+1,L_open_foo[1], x_goal, y_goal,L_open_foo[2])
        ...
        if(L_open_foo[4][L_open_foo[0]][L_open_foo[1]+1] != 0 and L_open_foo[4][L_open_foo[0]][L_open_foo[1]+1] < 200):
            heu=heuristic(L_open_foo[0],L_open_foo[1]+1, x_goal, y_goal,L_open_foo[2])
...
[lines, columns, map]=open_file('inputTest7.txt')#reads information given from the map
